...
async function fkuRick(){
	...
	for (let index in bittrex){
		...
		for (let yobitCoinPair in yobit){
			...
			// if difference is > 1%
			let biggestPercentDifference = 0
			if (yobitCoinPair === bittrexCoinPair
				&& (bittrexPrice - yobitPrice) > biggestPercentDifference
...
Yobit Volume: ${data.yobitVol}
Bittrex Volume: ${data.bittrexVol}
`
